...
export async function GET(req: NextRequest) {
  ...
  const items = await prisma.fAQItem.findMany({ where: { profileId }, orderBy: { position: "asc" } });
  return NextResponse.json({ ok: true, items });
}

export async function POST(req: NextRequest) {
  await requirePlan("PRO"); // gate edit ability at PRO+
  const session = await getServerSession(authOptions);
  if (!session?.user?.email) return NextResponse.json({ ok: false, message: "Unauthorized" }, { status: 401 });
  ...
  // Replace-all semantics (simpler ordering)
...
export async function DELETE(req: NextRequest) {
  ...
  const item = await prisma.fAQItem.findUnique({ where: { id }, include: { profile: { include: { user: true } } } });
